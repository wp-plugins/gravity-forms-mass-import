...
function impform_scripts_plug() {
	if(is_admin()){
		wp_register_script( 'backcallsc', plugins_url( 'js/backtocall.js', __FILE__ ), array('jquery'));			wp_enqueue_script('backcallsc');
...
function aad_callback() {
	...
	else {echo json_encode(array("sucs"=>"WRONG"));	}
	die();
}
...
function get_field_list_massimport($num){
	...
	for($i=0;$i<sizeof($ararr);$i++){
		$checkarey = array("section","page","html","fileupload","captcha","post_title","post_content","post_excerpt","post_tags","post_category","post_image","post_custom","product","quantity","option","shipping","total");
...
/*Just adding the admin settings page and putting the link in the gravity forms menu section*/
